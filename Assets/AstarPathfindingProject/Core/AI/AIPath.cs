...
namespace Pathfinding {
	...
	public partial class AIPath : AIBase, IAstarAI {
        ...
        /// <summary>
        /// Gets the next waypoint in the path, if available.
        /// </summary>
        public Vector3? GetNextWaypoint()
        {
            if (interpolator.valid && interpolator.remainingDistance > 0)
            {
                // Check if there is a next waypoint and return it
                if (interpolator.segmentIndex < path.vectorPath.Count - 1)
                {
                    return path.vectorPath[interpolator.segmentIndex + 1];
                }
            }
            return null;
        }
